...
#include <assert.h>
// FIXME: We remove malloc.h header due to conflict with
...
mbtree<P>::search_range_oid(const key_type &lower, const key_type *upper,
  ...
  // XXX: Seems I can just pass nullptr to xc, since
...
// XXX: This callback is for raw masstree(key=varstr, value=uinptr_t) use.
struct NoXIDSearchRangeCallback : public mtt::ConcurrentMasstree::low_level_search_range_callback {
  ...
  virtual bool invoke(const mtt::ConcurrentMasstree *btr_ptr,
                      ...
                      uint64_t version) {
      // FIXME: We don't need to implement this now.
