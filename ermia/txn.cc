...
rc_t transaction::si_commit() {
  ...
  // get clsn, abort if failed
  // NOTE: This call is still safe even we already finish 
...
rc_t transaction::Update(TableDescriptor *td, OID oid, const varstr *k, varstr *v) {
  ...
  if (prev_obj) {  // succeeded
    ...
    if (prev_clsn.asi_type() == fat_ptr::ASI_XID and
      ...
      // FIXME: 20190210: seems the deallocation here is too early,
    ...
    // FIXME: mark deleted in all 2nd indexes as well?

    // The varstr also encodes the pdest of the overwritten version.
    // FIXME: the pdest of the overwritten version doesn't belong to
