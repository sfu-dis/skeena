...
void MVCC::clone_oldest_view(ReadView *view) {
  ...
  /*  We can just do a atomic load from the counter, since the min_active_id is monotonically increasing */
  if (ermia_hton) {
    auto adjust_watermark = trx_sys->min_active_id.load();
    if (adjust_watermark) {
        view->reduce_low_limit(adjust_watermark - 1);
    }
  }
}
