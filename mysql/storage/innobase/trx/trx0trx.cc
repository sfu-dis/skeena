...
#include <mysql/thread_pool_priv.h>  // do_for_all_thd

#include <sql/gtt.h>  // cross-engine transaction
...
static void check_or_recycle();
/* Set the transaction to be cross-engine by acquiring an ERMIA begin timestamp */
void trx_register_cross_engine(trx_t *trx) {
  /* Ensure the ERMIA Engine is fully initialized */
  if (ermia::logmgr) {
    current_thd->inno_trx_id = trx->id;

    // Get an ERMIA read view if we don't have one yet
    if (!current_thd->begin_ermia_lsn) {
      current_thd->begin_ermia_lsn = ermia::logmgr->cur_lsn().offset();
    }
  }
}
    ...
    bool read_write) /*!< in: true if read-write transaction */
{
  ...
  if (!trx->read_only &&
    ...
    DLOG(INFO) << " [trx_start_low 1] Set trx id in thd " << trx->id;
    trx_register_cross_engine(trx);
    ...
    if (!trx_is_autocommit_non_locking(trx)) {
      ...
      if (read_write) {
        ...
        DLOG(INFO) << " [trx_start_low 2] Set trx id in thd " << trx->id;
        trx_register_cross_engine(trx);
                                        ...
                                        referred undo rseg. */
{
  ...
  // We don't need to handle the rollback case now, since the trx->no will get sorted in serialisation list later
  // And UNDO log doesn't seem necessary to be sorted for purging to work.
  if (ermia_hton && current_thd->serialization_no > 0 && !ermia_hton->gtt->is_bypass() /* && 
          !current_thd->transaction_rollback_request*/ ) {
    trx->no = current_thd->serialization_no;
  } else {
  ...
  if (!trx->read_only) {
    // We guarantee the serialisation list is sorted according to trx->no
    trx_t *tnode = nullptr;
    bool inserted = false;
    if(!UT_LIST_GET_LEN(trx_sys->serialisation_list)) {
      UT_LIST_ADD_LAST(trx_sys->serialisation_list, trx);
      inserted = true;
    } else {
#ifdef UNIV_DEBUG
      UT_LIST_CHECK(trx_sys->serialisation_list);
#endif
      trx_t *first = UT_LIST_GET_FIRST(trx_sys->serialisation_list);
      if(first->no > trx->no) {
          UT_LIST_ADD_FIRST(trx_sys->serialisation_list, trx);
          inserted = true;
      }
      for (tnode = UT_LIST_GET_FIRST(trx_sys->serialisation_list); tnode != NULL && !inserted ; tnode = UT_LIST_GET_NEXT(no_list, tnode)) {
          trx_t *nxt = UT_LIST_GET_NEXT(no_list, tnode);
          if(nxt == NULL || nxt->no > trx->no) {
              ut_list_insert(trx_sys->serialisation_list, tnode, trx);
              inserted = true;
              break;
          }
      }
...
written */
{
  ...
  if (trx_is_autocommit_non_locking(trx)) {
    ...
    if (trx->read_only || trx->rsegs.m_redo.rseg == NULL) {
      ...
      if (trx->read_view != NULL) {
        // XXX: Call view_close only if the view is in the list
        trx_sys->mvcc->view_close(trx->read_view, false);
      }
...
ReadView *trx_assign_read_view(trx_t *trx) /*!< in/out: active transaction */
{
  ...
  if (!MVCC::is_view_active(trx->read_view)) {
    // Create a new read view and adjust watermarks later as needed
    trx_sys->mvcc->view_open(trx->read_view, trx);

    // TODO: enable for isolation levels >= RC
    if (ermia_hton && !ermia_hton->gtt->is_bypass() && ermia::logmgr && trx->isolation_level == TRX_ISO_REPEATABLE_READ) {
      // Always obtain an ERMIA read view first
      auto &thd = current_thd;
      if (!thd->begin_ermia_lsn) {
        thd->begin_ermia_lsn = ermia::logmgr->cur_lsn().offset();
      }

      // Set trx id in thd to indicate this is a cross-container transaction that needs to insert
      // into GTT upon commit
      thd->inno_trx_id = trx->id;

      auto get_max_trx_id_fn = []() -> uint64_t {
        return trx_sys->max_trx_id;
      };

      uint64_t serialization_no = ermia_hton->gtt->get_innodb_trx_id(thd->begin_ermia_lsn, get_max_trx_id_fn);

      if (serialization_no) {
        if (serialization_no <= trx->read_view->m_low_limit_id) {

          // Returned a valid serialisation no, now construct a read view based on it by adjusting watermarks
          DLOG(INFO) << "Adjust m_low_limit_id(himark) from " << trx->read_view->m_low_limit_id << " to " << serialization_no;

          // Adjust the low water mark first to keep the constraint check of (lowmark <= highmark)
          if (trx->read_view->m_up_limit_id >= serialization_no) {
            DLOG(INFO) << "Adjust m_up_limit_id from(lo mark) " << trx->read_view->m_up_limit_id << " to " << serialization_no;
            trx->read_view->m_up_limit_id = serialization_no;
          }
          trx->read_view->m_low_limit_id = serialization_no;

          if (!thd->transaction_rollback_request && serialization_no < purge_sys->view.low_limit_no()) {
              DLOG(ERROR) << "himark we get is even smaller than purge_sys->view, should abort ,trx_id = " 
                  << serialization_no << " purge_view->low_m = " << purge_sys->view.low_limit_no();
              thd->transaction_rollback_request = true;
          }
        }
      } else {
        // Otherwise the transaction must abort - cannot acquire a valid readview
        // Mark the transaction to abort at the thd level
        // TODO: this allows the transaction to continue execution, but
        // will be aborted when it attempts to commit. Ideally we'd want to
        // avoid it doing any work at all to save cycles.
        thd->transaction_rollback_request = true;
      }
    }
  }

  ALWAYS_ASSERT(trx->read_view);
  return (trx->read_view);
}

static void check_or_recycle() {
  // gtt.recycle will acquire lock by itself
  if (ermia_hton && !ermia_hton->gtt->is_bypass() && ermia::logmgr) {
    uint64_t min_elsn = ~uint64_t{0};
    auto get_min_elsn = [](THD *thd, uint64_t out_min) {
      if (thd->is_system_thread()) {
          return;
      }
      if (thd->begin_ermia_lsn && thd->begin_ermia_lsn < *(uint64_t*)out_min) {
        *(uint64_t*)out_min = thd->begin_ermia_lsn;
      }
    };
    if (ermia_hton->gtt->get_do_recycle()) {
      do_for_all_thd(get_min_elsn, (uint64_t)&min_elsn);
      if (min_elsn != ~uint64_t{0}) {
        ermia_hton->gtt->recycle(min_elsn);
      }
    }
  }
}
...
void trx_set_rw_mode(trx_t *trx) /*!< in/out: transaction that is RW */
{
  ...
  DLOG(INFO) << " [trx_set_rw_mode] Set trx id in thd " << trx->id;
  trx_register_cross_engine(trx);
...
#ifdef UNIV_DEBUG
    if (ermia_hton && !ermia_hton->gtt->is_bypass()) {
      trx->read_view->print_limits(stdout);
    }
#endif
  }
