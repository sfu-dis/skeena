...
function other_ware(home_ware)
    ...
    until tmp ~= home_ware
    -- until tmp == home_ware
    -- print(string.format("tmp = %d, home_ware = %d", tmp, home_ware))
    return tmp
end

-- TODO: Change the get warehouse to follow SPEC.
function get_warehouse_id()
    -- If the warehouse number <= thread count, then do fixed warehouse
    -- If warehouse number > thread count, warehouse should distribute randomly
    
    if sysbench.opt.random_warehouse == false then
      if sysbench.opt.scale >= sysbench.opt.threads then
      	return sysbench.tid % sysbench.opt.scale + 1
      end
      error("When using fixed warehouse, warehouse number(scale) should be greater or equal than worker threads")
      return nil
    end

    ret = sysbench.rand.uniform(1, sysbench.opt.scale)	
    return ret
end
...
--                FOR UPDATE
  ...
  d_next_o_id, d_tax = con:query_row(([[SELECT d_next_o_id, d_tax 
                                          FROM district%d 
					 WHERE district_pkey = '%s' FOR UPDATE]]):
                                        format(table_num, string.format("%d/%d", w_id, d_id)))
...
--                AND d_w_id = :w_id;
  ...
  con:query(([[UPDATE district%d
                  SET d_next_o_id = %d
                WHERE district_pkey = '%s']]):format(table_num, d_next_o_id + 1, string.format("%d/%d", w_id, d_id)))
...
--                       :o_ol_cnt, :o_all_local);
  ...
  con:query(([[INSERT INTO orders%d
                           (orders_pkey, orders_w_d_c_id, o_id, o_d_id, o_w_id, o_c_id,  o_entry_d, o_ol_cnt, o_all_local)
                    VALUES ('%s', '%s', %d, %d, %d, %d, NOW(),%d, %d)]]):
                    format(table_num, 
		    	string.format("%d/%d/%d", w_id, d_id, d_next_o_id), 
		    	string.format("%d/%d/%d", w_id, d_id, c_id), 
		    	d_next_o_id, d_id, w_id, c_id, ol_cnt, all_local))

-- INSERT INTO new_orders (no_o_id, no_d_id, no_w_id)
--    VALUES (:o_id,:d_id,:w_id); */

  con:query(([[INSERT INTO new_orders%d (no_pkey, no_o_id, no_d_id, no_w_id)
                    VALUES ('%s', %d,%d,%d)]]):
                   format(table_num, string.format("%d/%d/%d", w_id, d_id, d_next_o_id), d_next_o_id, d_id, w_id))
...
--	FOR UPDATE;*/
	...
	s_quantity, s_data, ol_dist_info = con:query_row(([[SELECT s_quantity, s_data, s_dist_%s s_dist 
	                                                      FROM stock%d  
							     WHERE stock_pkey = '%s' FOR UPDATE]]):
	                                                     format(string.format("%02d",d_id),table_num,string.format("%d/%d",ol_supply_w_id,ol_i_id)))
...
--	AND s_w_id = :ol_supply_w_id;*/
	...
	con:query(([[UPDATE stock%d
	                SET s_quantity = %d
		      WHERE stock_pkey = '%s']]):
		    format(table_num, s_quantity, string.format("%d/%d",ol_supply_w_id,ol_i_id)))
...
--		:ol_dist_info);
	...
	con:query(([[INSERT INTO order_line%d
                                 (ol_pkey, ol_w_d_o_id, ol_o_id, ol_d_id, ol_w_id, ol_number, ol_i_id, ol_supply_w_id, ol_quantity, ol_amount, ol_dist_info)
	                  VALUES ('%s','%s',%d,%d,%d,%d,%d,%d,%d,%d,'%s')]]):
	                  format(
			        table_num,
			  	string.format("%d/%d/%d/%d", w_id, d_id, d_next_o_id, ol_number),
				string.format("%d/%d/%d", w_id, d_id, d_next_o_id),
...
function payment()
    -- payment identifier
    void_id = sysbench.rand.string("@@@-@@@-@@@-@@@")

    local table_num = sysbench.rand.uniform(1, sysbench.opt.tables)
    -- ALWAYS USE HOME WAREHOUSE
    -- local w_id = sysbench.tid % sysbench.opt.scale + 1
    local w_id = get_warehouse_id()
    local d_id = sysbench.rand.uniform(1, DIST_PER_WARE)
    local c_id = NURand(1023, 1, CUST_PER_DIST)
...
--  WHERE w_id =:w_id
    ...
    ret = con:query(([[UPDATE warehouse%d
   	          SET w_ytd = w_ytd + %d 
...
--		AND d_id = :d_id;*/
  ...
  con:query(([[UPDATE district%d 
                 SET d_ytd = d_ytd + %d 
	       WHERE district_pkey = '%s']]):format(table_num, h_amount, string.format("%d/%d", w_id, d_id)))
  ...
  d_street_1,d_street_2, d_city, d_state, d_zip, d_name = 
                          con:query_row(([[SELECT d_street_1, d_street_2, d_city, d_state, d_zip, d_name 
                                             FROM district%d
				       WHERE district_pkey = '%s']]):format(table_num, string.format("%d/%d", w_id, d_id)))
  -- Intentionally set byname to 0 --					   
  -- byname = 0
  if byname == 1 then
...
--	AND c_last = :c_last;*/
	...
	local namecnt = con:query_row(([[SELECT count(c_id) namecnt
			                   FROM customer%d
			                    WHERE cust_w_d_id = '%s'
                                            AND c_last='%s']]):format(table_num, string.format("%d/%d", c_w_id, c_d_id), c_last ))
--		SELECT c_id
--		FROM customer
--		WHERE c_w_id = :c_w_id 
--		AND c_d_id = :c_d_id 
...
--		ORDER BY c_first;
	...
	rs = con:query(([[SELECT c_id
			...
			):format(table_num, string.format("%d/%d", c_w_id, c_d_id), c_last ))

	for i = 1,  (namecnt / 2) do
		row = rs:fetch_row()
		c_id = row[1]
	end
  end -- byname
...
--	FOR UPDATE;
  ...
  -- print(tprint(sysbench))
  -- print(tprint(sysbench.opt.mysql_ignore_errors))
  ...
  c_phone, c_credit, c_credit_lim, c_discount, c_balance, c_ytd_payment, c_since =
	 con:query_row(([[SELECT c_first, c_middle, c_last, c_street_1,
			    ...
			    WHERE cust_pkey = '%s'
			      FOR UPDATE]])
			 :format(table_num, string.format("%d/%d/%d", c_w_id, c_d_id, c_id )))
...
-- 	AND c_id = :c_id; */
    ...
    --			AND c_id = :c_id
        con:query(([[UPDATE customer%d
                      ...
                      WHERE cust_pkey = '%s']])
		  :format(table_num, c_balance, c_ytd_payment, c_new_data, string.format('%d/%d/%d', c_w_id, c_d_id, c_id)))
   else
        con:query(([[UPDATE customer%d
                        SET c_balance=%f, c_ytd_payment=%f
                      WHERE cust_pkey = '%s'
                        ]])
		  :format(table_num, c_balance, c_ytd_payment, string.format('%d/%d/%d', c_w_id, c_d_id, c_id)))
...
-- TODO: Insert to History pending now.
...
--            AND c_last = :c_last;*/
        ...
        namecnt = con:query_row(([[SELECT count(c_id) namecnt
				     ...
				     WHERE cust_w_d_id = '%s'
                                      AND c_last='%s']]):
                                  format(table_num, string.format("%d/%d", w_id, d_id), c_last ))
...
--			AND c_id = :c_id;*/
        c_balance, c_first, c_middle, c_last = 
                   con:query_row(([[SELECT c_balance, c_first, c_middle, c_last
                                      FROM customer%d
				      WHERE cust_pkey = '%s']])
                                  :format(table_num, string.format("%d/%d/%d", w_id, d_id, c_id )))
    end
--[=[ Initial query
...
--		AND ol_o_id = :o_id;*/
    ...
    rs = con:query(([[SELECT ol_i_id, ol_supply_w_id, ol_quantity, ol_amount, ol_delivery_d
            FROM order_line%d WHERE ol_w_d_o_id = '%s']])
                  :format(table_num, string.format("%d/%d/%d", w_id, d_id, o_id)))
    for i = 1,  rs.nrows do
        row = rs:fetch_row()
        local ol_i_id = row[1]
...
--		  AND no_w_id = :w_id;*/
	...
	-- TODO: Seems the delete operation is not supportted by MySQL ERMIA engine, what should we do
        con:query(([[UPDATE new_orders%d SET deleted = 1 WHERE no_pkey = '%s']])
                            :format(table_num, string.format("%d/%d/%d", w_id, d_id, no_o_id)))
...
--				o_w_id = :w_id;*/
        ...
        con:query(([[UPDATE orders%d 
                        SET o_carrier_id = %d
		      WHERE orders_pkey = '%s']])
                      :format(table_num, o_carrier_id, string.format("%d/%d/%d", w_id, d_id, no_o_id)))

--   UPDATE order_line
--		                SET ol_delivery_d = :datetime
--		                WHERE ol_o_id = :no_o_id AND ol_d_id = :d_id AND
--				ol_w_id = :w_id;*/
        -- XXX: Do we support secondary index update?
    ...
    ---                   :format(table_num, string.format("%d/%d/%d", w_id, d_id, no_o_id)))
    ---- BEGIN COMMENT -----
	res = con:query(([[SELECT ol_pkey FROM order_line%d WHERE ol_w_d_o_id = '%s']])
		:format(table_num, string.format("%d/%d/%d", w_id, d_id, no_o_id)))
	pkey_ids = {}
	pkey_id_size = res.nrows
	...
	for i = 1, pkey_id_size do
        	con:query(([[UPDATE order_line%d 
                        SET ol_delivery_d = NOW()
			WHERE ol_pkey = '%s']])
                      :format(table_num, pkey_ids[i]))
    end
    ---- END COMMENT -----
...
--        print(string.format("update customer table %d, cid %d, did %d, wid %d balance %f",table_num, o_c_id, d_id, w_id, sm_ol_amount))  				
        con:query(([[UPDATE customer%d 
                        SET c_balance = c_balance + %f,
                            c_delivery_cnt = c_delivery_cnt + 1
                      WHERE cust_pkey = '%s']])
                      :format(table_num, sm_ol_amount, string.format("%d/%d/%d", w_id, d_id, o_c_id)))
        end
...
-- TODO: How to change this to use pkey also?
-- TODO: Need to use manual join here, write a loop
    -- Manual Join Starts here.
    -- print("START MANUAL JOIN")
    size = 0
    rows = {}
    cnt = 1
    -- Change the range to be a loop
    for ol_o_id = d_next_o_id - 20, d_next_o_id - 1 do
      res = con:query(([[SELECT ol_i_id FROM order_line%d WHERE ol_w_d_o_id = '%s']])
    		  :format(table_num, string.format("%d/%d/%d", w_id, d_id, ol_o_id)))
      size = size + res.nrows
      for i = 1, res.nrows do
        rows[cnt] = res:fetch_row()
	cnt = cnt + 1
      end
    end
    -- print("FINISH FETCH")
    ...
    for i = 1, size do
      ol_i_id = rows[i][1]
      tmp = con:query_row(([[SELECT COUNT(DISTINCT(s_i_id)) FROM stock%d WHERE stock_pkey = '%s' AND s_quantity < %d]]):format(table_num, string.format("%d/%d", w_id, ol_i_id), level))
      -- print(ol_i_id, tmp, rs)
      rs = rs + tmp
    end
...
--                      AND s_quantity < :level;*/
        ...
        rs = con:query(([[SELECT count(*) FROM stock%d
                   WHERE stock_pkey = '%s'
                   AND s_quantity < %d]])
                :format(table_num, string.format("%d/%d", w_id, ol_i_id[i]), level ) )
        local cnt
        for i = 1, rs.nrows do
