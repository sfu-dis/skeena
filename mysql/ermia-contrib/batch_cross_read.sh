...
do
  ...
  do
    ...
    taskset -c <client threads> sysbench ./sysbench/oltp_read_only_cross.lua --mysql-ignore-errors=1180,1030,1412 --innodb_percentage=$j --mysql_storage_engine=ERMIA --create_secondary=off --rand-type=uniform --tables=250 --range_selects=off --table_size=250000 --mysql-db=dbtest --db-driver=mysql --mysql-socket=/dev/shm/$USER/mysql/mysqld.sock --threads=$i --time=30 --report-interval=1 run > ./sysbench/sysbench-log/cross_read_only_${i}_innodb${j}.log
  done
done
