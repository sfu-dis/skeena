...
sysbench.cmdline.options = {
   ...
   tables =
      {"Number of tables", 1},
   innodb_percentage =
      {"Number of queries on InnoDB tables per transaction", 50},
   point_selects =
...
local stmt_defs = {
   point_selects_innodb ={
      "SELECT c FROM sbtest%u_innodb WHERE id=?",
      t.INT},
   point_selects_ERMIA ={
   ...
   distinct_ranges = {
      ...
      t.INT, t.INT},
   index_updates_innodb = {
      "UPDATE sbtest%u_innodb SET k=k+1 WHERE id=?",
      t.INT},
   index_updates_ERMIA = {
   ...
   index_updates = {
      ...
      t.INT},
   non_index_updates_innodb = {
      "UPDATE sbtest%u_innodb SET c=? WHERE id=?",
      {t.CHAR, 120}, t.INT},
   ...
   non_index_updates = {
      ...
      {t.CHAR, 120}, t.INT},
   deletes_innodb = {
      "DELETE FROM sbtest%u_innodb WHERE id=?",
      t.INT},
   deletes_ERMIA = {
   ...
   deletes = {
      ...
      t.INT},
   inserts_innodb = {
      "INSERT INTO sbtest%u_innodb (id, k, c, pad) VALUES (?, ?, ?, ?)",
...
function prepare_point_selects()
   prepare_for_each_table("point_selects_innodb")
...
function prepare_index_updates()
   prepare_for_each_table("index_updates_innodb")
   prepare_for_each_table("index_updates_ERMIA")
end

function prepare_non_index_updates()
   prepare_for_each_table("non_index_updates_innodb")
   prepare_for_each_table("non_index_updates_ERMIA")
end

function prepare_delete_inserts()
   prepare_for_each_table("deletes_innodb")
   prepare_for_each_table("inserts_innodb")
...
function execute_point_selects()
   local tnum_innodb = get_table_num()
   local tnum_ERMIA = get_table_num()
   local i
   local innodb_queries = math.ceil((sysbench.opt.innodb_percentage / 100) * sysbench.opt.point_selects)
   local which_engine = {}
   
   for i = 1, innodb_queries do
      which_engine[i] = 1
   end
   
   for i = innodb_queries + 1, sysbench.opt.point_selects do
      which_engine[i] = 2
   end
   ...
   for i = 1, sysbench.opt.point_selects do
      -- 1 is innodb, 2 is ERMIA.
      if which_engine[i] == 1 then
         param[tnum_innodb].point_selects_innodb[1]:set(get_id())
         
         stmt[tnum_innodb].point_selects_innodb:execute()
      else
...
function execute_index_updates()
   local tnum_innodb = get_table_num()
   local tnum_ERMIA = get_table_num()
   local innodb_queries = math.ceil((sysbench.opt.innodb_percentage / 100) * sysbench.opt.index_updates)
   local which_engine = {}
   ...
   for i = innodb_queries + 1, sysbench.opt.index_updates do
      which_engine[i] = 2
   end
   ...
   for i = 1, sysbench.opt.index_updates do
      -- 1 is innodb, 2 is ERMIA.
      if which_engine[i] == 1 then
         param[tnum_innodb].index_updates_innodb[1]:set(get_id())
         
         stmt[tnum_innodb].index_updates_innodb:execute()
      else
...
function execute_non_index_updates()
   local tnum_innodb = get_table_num()
   local tnum_ERMIA = get_table_num()
   local innodb_queries = math.ceil((sysbench.opt.innodb_percentage / 100) * sysbench.opt.non_index_updates)
   local which_engine = {}
   ...
   for i = innodb_queries + 1, sysbench.opt.non_index_updates do
      which_engine[i] = 2
   end
   ...
   for i = 1, sysbench.opt.non_index_updates do
      -- 1 is innodb, 2 is ERMIA.
      if which_engine[i] == 1 then
         param[tnum_innodb].non_index_updates_innodb[1]:set_rand_str(c_value_template)
         param[tnum_innodb].non_index_updates_innodb[2]:set(get_id())
         
         stmt[tnum_innodb].non_index_updates_innodb:execute()
      else
...
function execute_delete_inserts()
   local tnum_innodb = get_table_num()
   local tnum_ERMIA = get_table_num()
   local innodb_queries = math.ceil((sysbench.opt.innodb_percentage / 100) * sysbench.opt.delete_inserts)
   local which_engine = {}
   ...
   for i = innodb_queries + 1, sysbench.opt.delete_inserts do
      which_engine[i] = 2
   end
   ...
   for i = 1, sysbench.opt.delete_inserts do
      ...
      if which_engine[i] == 1 then
         param[tnum_innodb].deletes_innodb[1]:set(id)

         param[tnum_innodb].inserts_innodb[1]:set(id)
         param[tnum_innodb].inserts_innodb[2]:set(k)
         param[tnum_innodb].inserts_innodb[3]:set_rand_str(c_value_template)
         param[tnum_innodb].inserts_innodb[4]:set_rand_str(pad_value_template)

         stmt[tnum_innodb].deletes_innodb:execute()
         stmt[tnum_innodb].inserts_innodb:execute()
      else
