...
extern int ermia_real_commit(handlerton *hton, THD *thd);
...
int ha_commit_low(THD *thd, bool all, bool run_after_commit) {
  ...
  if (ha_info) {
    ...
    */
    ...
    // ERMIA engine commits first, find if ermia involves in this trx
    bool do_ermia_commit = false;
    for (auto *p = ha_info; p ; p = p->next()) {
      if (p->ht()->db_type == DB_TYPE_ERMIA) {
        do_ermia_commit = true;
        break;
      }
    }

    // Do the commit check if this is a "truly" cross-engine transaction,
    // i.e., involves not only acquiring a readview from ERMIA, but actually
    // did read/write accesses in ERMIA. I.e., InnoDB-only transactions (which
    // has ERMIA readview but never used it) do not need this commit check.
    if (do_ermia_commit) {
      // TODO: Commit will always succeed, no return value,
      // need to handle error if failed
      if (!thd->transaction_rollback_request) {
        auto retval = ermia_hton->commit(ermia_hton, thd, all);
        ALWAYS_ASSERT(retval == 0);
        ALWAYS_ASSERT(thd->ermia_pre_commit_lsn);
      }

      if (thd->inno_trx_id && !thd->transaction_rollback_request) {
        // Use the larger of <begin LSN, commit LSN> as commit LSN could be 0
        uint64_t elsn = std::max(thd->ermia_pre_commit_lsn, thd->begin_ermia_lsn);
        ALWAYS_ASSERT(elsn);

        if (!ermia_hton->gtt->is_bypass()) {
          // acquire the innodb id here
          auto sn = innodb_hton->pre_commit();
          DLOG(INFO) << "Cross-commit check " << elsn << ", " << sn;
          thd->serialization_no = sn;
          bool ok = ermia_hton->gtt->commit_check(elsn, sn);
          DLOG(INFO) << "Cross-engine transaction can commit: " <<  ok;
          if (!ok) {
      ...
      if (thd->transaction_rollback_request) {
        ...
        int err = HA_ERR_COMMIT_PROTOCOL_ABORT;
        my_error(ER_ERROR_DURING_COMMIT, MYF(0), err,
                 my_strerror(errbuf, MYSQL_ERRMSG_SIZE, err));
        error = 1;
        ermia_hton->rollback(ermia_hton, thd, all);
      }
    }
    ...
    for (; ha_info; ha_info = ha_info_next) {
      ...
      if (ha_info->ht()->db_type == DB_TYPE_INNODB) {
        if (thd->transaction_rollback_request) {
          ...
          err = HA_ERR_COMMIT_PROTOCOL_ABORT;
        }
        if (err == HA_ERR_COMMIT_PROTOCOL_ABORT || (err = ht->commit(ht, thd, all))) {
          char errbuf[MYSQL_ERRMSG_SIZE];
          my_error(ER_ERROR_DURING_COMMIT, MYF(0), err,
                   my_strerror(errbuf, MYSQL_ERRMSG_SIZE, err));
          error = 1;
    ...
    if (do_ermia_commit && thd->inno_trx_id) {
      // If this is a not cross container transaction, we already do the real_commit inside
      // ermia->commit, should skip this commit part but the cleanup work also need to be done
      if (!thd->transaction_rollback_request) {
        ermia_real_commit(ermia_hton, thd);
      }
      thd->status_var.ha_commit_count++;
      if (restore_backup_ha_data) reattach_engine_ha_data_to_thd(thd, ermia_hton);
    }

    // Clean up the trx info after use
    if (all) {
    	thd->inno_trx_id = 0;
    	thd->begin_ermia_lsn = 0;
        thd->serialization_no = 0;
    }
