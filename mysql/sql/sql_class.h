...
class THD : public MDL_context_owner,
 ...
 public:
  ...
  // TODO: Currently we use uint64_t to store the lsn, later change it to a general type LSN.
  // Also has its general LSN comparation, retriving fns
  // TODO: Change this to a structure contains several commit_lsn
  ermia::rLSN rlsn;

  // To keep the sequential behaviour under pipelined commit
  bool pipeline_notified = true;

  // Store a transaction wide begin lsn for each time the txn start(It's a pure ermia lsn(timestamp))
  // Use 0x0 as the invalid ermia lsn
  uint64_t begin_ermia_lsn = 0x0;

  // Store the innodb transaction id for commit protocol verification (non zero means this is an read-write transaction
  uint64_t inno_trx_id = 0x0;
  uint64_t serialization_no = 0x0;

  // Store the ermia_pre_commit lsn for commit protocol verification
  uint64_t ermia_pre_commit_lsn = 0x0;
