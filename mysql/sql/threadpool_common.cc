...
std::atomic<uint32_t> worker_count(0);
thread_local uint32_t my_worker_id = ~uint32_t{0};
...
void mysql_post_commit_callback(void *context) {
  THD *thd = (THD *)context;

  // FIXME: to be confirmed - this should be for debug only
  // current_thd = thd;

  // FIXME: clone_cmd and log_slow_performance (per-stmt counter) not
  // supported
  ALWAYS_ASSERT(!ermia::volatile_read(thd->pipeline_notified));
  dispatch_command_bottom_half(thd, COM_QUERY, nullptr, nullptr);
  do_command_bottom_half(thd);

  auto *vio = thd->get_protocol_classic()->get_vio();
  if (!vio->has_data(vio)) {
  ...
  ermia::volatile_write(thd->pipeline_notified, true);
}
...
int threadpool_process_request(THD *thd) {
  ...
  Worker_thread_context worker_context;
  if (my_worker_id == ~uint32_t{0}) {
    my_worker_id = worker_count.fetch_add(1) % ermia::config::worker_threads;
  }
  ...
  */
  for (;;) {
    ...
    while (!ermia::volatile_read(thd->pipeline_notified)) {}
    if ((retval = do_command(thd, false, &early_return)) != 0) goto end;

    if (early_return) {
      // Sometimes the lsn is 0 since it's not a commit related to ERMIA but still goes in early return
      ermia::volatile_write(thd->pipeline_notified, false);
      ermia::logmgr->enqueue_committed_xct(my_worker_id, thd->rlsn, 0, mysql_post_commit_callback, (void *)thd);
      return 0;
    } else {
      ALWAYS_ASSERT(ermia::volatile_read(thd->pipeline_notified));
      if (!thd_connection_alive(thd)) {
