...
static inline void pin_thread(unsigned int worker_id) {
    auto this_thread = pthread_self();
    cpu_set_t cpus;
    CPU_ZERO(&cpus);
    // MySQL will create two threads for each worker, so we pin them on the same phy core
    auto phy_core = ermia::thread::cpu_cores[(worker_id / 2) 
        % ermia::thread::cpu_cores.size()].physical_thread;
    CPU_SET(phy_core, &cpus);
    pthread_setaffinity_np(this_thread, sizeof(cpu_set_t), &cpus);
    return ;
}

std::atomic_uint worker_id{0};

static void *worker_main(void *param) {
  my_thread_init();

  DBUG_ENTER("worker_main");
  LOG(INFO) << "create worker thread id = " << ulong(pthread_self()) % 40 << 
      ", worker_id: " << worker_id.fetch_add(1, std::memory_order_relaxed);
  ...
  ermia::logmgr->free_tls_lsn_slot();
...
bool tp_init() {
  ...
  if (!ermia::thread::cpu_cores.size()) {
      ermia::thread::DetectCPUCores();
      LOG(INFO) << "cpu_cores size: " << ermia::thread::cpu_cores.size();
  }
